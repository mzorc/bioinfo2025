...
# b) Using code:
# setwd("C:/Users/mario/Desktop/MATE2024 Population Genomics Workshop/Day 2/3_Estimation of Inbreeding and Relatedness/2_Practical excercises/Example2/Directory")


## Then we import data (using data.table package) 
# install.packages('data.table')
library(data.table)
map_file <- as.data.frame(fread(file = 'CROholstein_FinalReport_afterQC.map',header = FALSE))
ped_file <- as.data.frame(fread(file = 'CROholstein_FinalReport_afterQC.ped',header = FALSE))
...
genotypeFilePath <- 'CROholstein_FinalReport_afterQC.ped'
mapFilePath <- 'CROholstein_FinalReport_afterQC.map'
...
ROHs <- consecutiveRUNS.run(genotypeFile = genotypeFilePath, mapFile = mapFilePath, minSNP = 15, ROHet = FALSE, 
                            maxGap = 1000000, minLengthBps = 4000000, maxOppRun = 1, maxMissRun = 1)
# minLengthBps = 4000000 --> 50K SNP array
...
###                                               into length categories (4-8 Mb, 8-16 Mb, >16 Mb)
...
###  5.a) Froh estimation from 4 to 8 Mb (from approximately 12.5 to 6.25 generations ago)
...
###  5.b) Froh estimation from 8 to 16 Mb (from approximately 6.25 to 3.125 generations ago)
...
###  5.c) Froh estimation >16 Mb (from approximately 3.125 to 1 generations ago)
...
Froh_test <- cbind(Froh_test_allId_4to8Mb[,3],Froh_test_allId_8to16Mb[,3],Froh_test_allId_from16Mb[,3])
colnames(Froh_test) <- c('Froh_4to8Mb','Froh_8to16Mb','Froh_from16Mb')
...
# Example: Chromosome 10 only (using all chromosomes would take too long for the workshop)
Example_I_Index <- I_matrix_test
colnames(Example_I_Index) <- c(10,map_file$V1)
Regional.Froh_test_meanFroh_NormAll <- Regional.Froh.estimation(map = map_file[map_file$V1 == 10,], ped = ped_file, I_matrix = Example_I_Index[,colnames(Example_I_Index) == 10], return = 1, by.fid = FALSE, normalization = 'all', slide = 500000, end = 10000000)
...
Froh_forPlot$ROH <- factor(Froh_forPlot$ROH, levels = c(">16 Mb", "8-16 Mb", "4-8 Mb"))


## Visualization 

# Creating boxplot
px <- ggplot(Froh_test_allId, aes(x = factor(fid, levels = c('HOL')), 
                                  y = Froh, fill = fid)) +
  ...
  xlab("") +
  ggtitle("Boxplot for Holstein")
px

# Creating stacked plot
py <- ggplot(Froh_forPlot, aes(x = factor(fid, levels = c('HOL')), 
                               fill = ROH, y = Froh)) +
  geom_bar(stat = "summary", fun = "mean") +
  scale_fill_manual(values = c("yellow", "gray60", "black")) +
  theme_classic() +
  xlab("") +
  ylab(expression("F"["ROH"])) +
  ggtitle("Stacked Plot for Holstein")
py
...
system("plink.exe --file CROholstein_FinalReport_afterQC --nonfounders --output-missing-genotype 0 --recode12 --out Recoded_for_VanRadenGRM --cow")
...
vanRaden <- calc_gnrm(genofile='Recoded_for_VanRadenGRM.ped',genoformat="ped",ana_type="vanRaden",ped_option=F,
                      outputformat="matrix",outputname="GRM",nIID=120,missinggeno=T,plots=T)
...
# 1. Which function are we using in this workshop for ROH determination?

# 2. Which argument of this function has a different value in Example2 compared to the previous examples, and why?
