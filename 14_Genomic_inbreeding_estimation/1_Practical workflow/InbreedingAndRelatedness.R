...
# b) Using code:
# setwd("C:/Users/mario/Desktop/MATE2024 Population Genomics Workshop/Day 2/3_Estimation of Inbreeding and Relatedness/1_Practical workflow/Directory")


## Then we import data (using data.table package) 
# install.packages('data.table')
library(data.table)
map_file <- as.data.frame(fread(file = 'CROsheep_FinalReport_afterQC.map',header = FALSE))
ped_file <- as.data.frame(fread(file = 'CROsheep_FinalReport_afterQC.ped',header = FALSE))
...
genotypeFilePath <- 'CROsheep_FinalReport_afterQC.ped'
mapFilePath <- 'CROsheep_FinalReport_afterQC.map'
...
# Example: Chromosome 26 only (using all chromosomes would take too long for the workshop)
Example_I_Index <- I_matrix_test
colnames(Example_I_Index) <- c(26,map_file$V1)
Regional.Froh_test_meanFroh_NormAll <- Regional.Froh.estimation(map = map_file[map_file$V1 == 26,], ped = ped_file, I_matrix = Example_I_Index[,colnames(Example_I_Index) == 26], return = 1, by.fid = FALSE, normalization = 'all', slide = 500000, end = 10000000)
...
# Creating boxplot
px <- ggplot(Froh_test_allId, aes(x = factor(fid, levels = c('CRESKA', 'LICKA', 'PASKA', 'RAPSKA', 'KRCKA')), 
                                  y = Froh, fill = fid)) +
  geom_boxplot(alpha = 0.4) +
  scale_fill_manual(values = c("lightpink2", "cadetblue1", "mediumpurple4", "darkgreen", "orange")) +
...
py <- ggplot(Froh_forPlot, aes(x = factor(fid, levels = c('CRESKA', 'LICKA', 'PASKA', 'RAPSKA', 'KRCKA')), 
                               fill = ROH, y = Froh)) +
...
system("plink.exe --file CROsheep_FinalReport_afterQC --nonfounders --output-missing-genotype 0 --recode12 --out Recoded_for_VanRadenGRM --sheep")
...
#                       outputformat="matrix",outputname="GRM",nIID=100,missinggeno=T,plots=T)

# vanRaden

# Due to long runtime, for example we will use pruned data created with Plink:
system("plink.exe --file Recoded_for_VanRadenGRM --indep-pairwise 50 5 0.1 --out pruned_data --sheep")
system("plink.exe --file Recoded_for_VanRadenGRM --extract pruned_data.prune.in --recode --out reduced_genotype --sheep")
# This command uses a sliding window of 50 SNPs, shifts the window by 5 SNPs at a time and removes SNPs with an r^2 value above 0.1
